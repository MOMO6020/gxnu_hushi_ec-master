 ...
 * 用于防止发送空数据包
 */
bool group_enable_flag[8] = {false, false, false, false, false, false, false, false};   //用于标志八个电机对应八个CAN发送组，是否有电机需要控制
    ...
    : IMotor(_motor_type, _setting, _pid_config), motor_id_(_motor_id)
{
    ...
    switch (motor_type_)                                     //根据电机类型设置发送组和减速比                   
    {
        case MotorType::M2006:
        case MotorType::M3508:
            motor_tx_group_ = motor_id_ <= 4 ? 0 : 1;       //前四个id为组0，后四个为组1
            motor_tx_group_ = _hcan->Instance == CAN1 ? motor_tx_group_ : motor_tx_group_ + 4;  //CAN1选择0-3组，CAN2选则5-7组
            reduce_rate_    = motor_type_ == MotorType::M2006 ? 36 : 19;        //减速比
            break;
        case MotorType::GM6020:
            motor_tx_group_ = _hcan->Instance == CAN1 ? 2 : 3;    //GM6020在CAN1上为组2和组3
            motor_tx_group_ = motor_id_ <= 4 ? motor_tx_group_ : motor_tx_group_ + 4;       //同上
            reduce_rate_    = 1.0f;                 //GM6020无减速箱，但是会有摩擦
            break;
    ...
    registered_motors_[motor_count_++] = this;  // 添加到电机数组  //并增加电机计数
    group_enable_flag[motor_tx_group_] = true;  // 设置使能标志
}

void DJIMotor::decode(const uint8_t* buf, const uint8_t len)      //解码函数，处理CAN接收数据
{
    if (buf == nullptr || len < 8)                  //错误处理，缓冲区为空指针或者长度小于8
    {
        LOGERROR("DJIMotor", "Invalid buffer");
        return;
    }
    if (!is_online)                                 //如果之前离线，现在上线，打印上线信息？
    {
        is_online = true;
        switch (motor_type_)                        //根据电机类型打印日志
        {
    ...
    daemon_->feed();                                                     //喂狗，表示电机在线
    measure_.last_encoder = measure_.encoder;
    measure_.encoder      = ((uint16_t)buf[0] << 8) | buf[1];            //更新编码器值
    measure_.angle        = (float)measure_.encoder / 8192.0f * 360.0f;  //计算角度（0-360）
    measure_.speed        = (buf[2] << 8) | buf[3];                      //读取原始速度值并转换为度/秒
    measure_.speed_dps =
        (1.0f - SPEED_SMOOTH_COEF) * measure_.speed_dps + 6.0f * SPEED_SMOOTH_COEF * (float)measure_.speed; //对电机转速进行平滑滤波
    measure_.torque_current = (1.0f - CURRENT_SMOOTH_COEF) * measure_.torque_current +
                              CURRENT_SMOOTH_COEF * (float)((int16_t)(buf[4] << 8 | buf[5]));           //读取转矩电流并进行平滑滤波
    measure_.temperature = buf[6];                                              //电机温度
    if (measure_.encoder - measure_.last_encoder > 4096)                       
        measure_.total_round--;
    else if (measure_.last_encoder - measure_.encoder > 4096)
        measure_.total_round++;                                                 //处理圈数计数（编码器溢出处理） 
    measure_.total_angle = measure_.total_round * 360.0f + measure_.angle;      //计算总角度
}

void DJIMotor::offlineCallback()            //离线回调函数
{
    if (is_online)
    {
        memset(static_cast<void*>(&measure_), 0, sizeof(measure_));     //清零测量数据
        is_online = false;
        switch (motor_type_)            //打印离线警告
        {
...
int16_t DJIMotor::calculateOutputCurrent()      //计算输出电流
{
    if (!is_online || !enable_)
        return 0;

    // 1. 同步当前角度（从测量数据获取）
    current_angle_ = measure_.total_angle;

    // 2. 软件限位处理
    if (enable_limit_) {
        target_angle_ = std::clamp(target_angle_, min_angle_, max_angle_);
    }

    // 3. 将目标角度作为PID参考值
    float pid_ref = target_angle_;
...
/////// 位置环计算（使用目标角度作为参考）//////////////////////////////////////////////////////////////////////////////////////
    if ((setting_.close_loop & CloseloopType::ANGLE_LOOP) && setting_.outer_loop == CloseloopType::ANGLE_LOOP) {
        if (setting_.external_angle_feedback == FeedbackType::EXTERNAL) {
            ...
            pid_measure = current_angle_;  // 使用当前角度作为反馈
        }
        // 位置环PID输出作为速度环参考
...
/////////////////////////////////////////////////////////////////
    ...
    {
        if (setting_.external_speed_feedback == FeedbackType::EXTERNAL)     //反馈源选择
